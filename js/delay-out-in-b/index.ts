...
const exp: [
    ...
    async function(input: Streamable<any>, timesOfExecution = 12000) {
        ...
        await (input as StringStream)
            .while(o => {
                h = rht.bigint();  // Get the current local timestamp
                diff = h - BigInt(o);  // Calculate the difference between local timestamp and received timestamp
        ...
        diffs.splice(0, CUTOFF);  // Remove the initial cutoff entries from the differences array
